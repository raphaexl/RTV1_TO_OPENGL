...
/*   Updated: 2019/10/06 12:45:33 by ebatchas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int    ft_process_input(GLFWwindow *win, t_cam *cam, float delta)
{
    ...
    else if (glfwGetKey(win, GLFW_KEY_Q) == GLFW_PRESS && (ret = 1))
    direction = FORWARD;
    else if (glfwGetKey(win, GLFW_KEY_W) == GLFW_PRESS && (ret = 1))
    direction = BACKWARD;
    else if (glfwGetKey(win, GLFW_KEY_UP) == GLFW_PRESS && (ret = 1))
    direction = FORWARD;
    else if (glfwGetKey(win, GLFW_KEY_DOWN) == GLFW_PRESS && (ret = 1))
    direction = BACKWARD;
    else if (glfwGetKey(win, GLFW_KEY_A) == GLFW_PRESS && (ret = 1))
    direction = RIGHT;
    else if (glfwGetKey(win, GLFW_KEY_D) == GLFW_PRESS && (ret = 1))
    direction = LEFT;
